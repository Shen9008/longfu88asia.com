...
GitHub Actions (`.github/workflows/daily-sync.yml`) runs `npm run sync:daily` on schedule and on manual dispatch.
...
| `synced_at` | When the static site last wrote this entry (sort key for blog index) |
...
| `.github/workflows/daily-sync.yml` | Daily CI sync + deploy |
| `package.json` | `sync:daily`, `sync:refresh`, `sync:force` scripts |
