...
### Visitor-facing strings (`blog-loader.js`)

Text shown in **`#blog-posts-grid`** when the listing is empty or when **`blogs.json`** fails to load is **public-facing**. Do **not** mention **`npm run sync`**, Strapi, **`.env`**, repository paths, CI, or other setup-only concepts there—those belong in this doc or internal onboarding.

Use neutral wording only, for example:

- Empty list: “No articles yet. Check back soon.” (or variant matching your brand voice).
- Failed fetch: short apology + retry later, **without** file paths unless the site is a private admin tool.

Keeping dev hints out of shipped HTML avoids confusing visitors and leaking implementation detail when the template is copied to the next website.
