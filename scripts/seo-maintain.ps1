...
<#
.SYNOPSIS
  longfu88asia.com — SEO maintenance: internal link, local asset, and sitemap parity checks (static site).
...
.DESCRIPTION
  ...
  External URLs (e.g. affiliate, GTM) are skipped unless they use host longfu88asia.com.
...
function Resolve-SiteUrlToPath {
    ...
    $p = $UrlPath -replace '^https://longfu88asia\.com', '' -replace '#.*$', '' -replace '\?.*$', ''
    if ($p -eq "" -or $p -eq "/") {
...
foreach ($file in $htmlFiles) {
    ...
    foreach ($m in [regex]::Matches($text, $attrPattern)) {
        ...
        if ($val -match '^https://longfu88asia\.com') {
